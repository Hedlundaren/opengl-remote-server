...
void Sphere::create_sphere() {
	...
	for (int j = 0; j < res_phi; j++) {
		...
		uvs.push_back(glm::vec2(phi_stepSize * j / (2 * PI), 1.0f)); // top
		uvs.push_back(glm::vec2((phi_stepSize * j) / (2 * PI), 1.0f - theta_stepSize / PI)); // a
		uvs.push_back(glm::vec2((phi_stepSize * j + phi_stepSize) / (2 * PI), 1.0f - theta_stepSize / PI)); // b 
